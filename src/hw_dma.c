...
void DMA1_Channel3_IRQHandler()	{			//SPI1 DMA TC (Transfer Complete)
	...
	if(exclusiveDMATransmitt)	{
		//ILI9341ChipSelect(1);
//			SPI1->CR1 = 	0xB04;	//default state
//		SPI1->CR1 |= SPI_CR1_SPE;
			ILI9341SendCmd(0x3A);
	ILI9341SendShort(0x5500);
		exclusiveDMATransmitt = 0;
		return;
	}
//	if(task->countOfData <= task->transmittedData)
//		dataLast = 0;
//	else
...
//			__ASM("nop");
		...
		if(currentDMATask + 1 == DMA_MAX_TASK_COUNT)
			asm("nop");
		//x = (int) ((struct dmaTask*) (dmaTaskQuery + i)->operation);								//if next task has any job
//		if( (x == (int) Free ) || x == (int)Blocked)
//			return;
		task = getDMATask();
		if(task == (struct _dmaTask*) 0)
			return;
...
//				pX = task->service.terminalWindow->posX;// task->service.terminalWindow->position.xS;//current pixel position
//				pY = task->service.terminalWindow->posY;// + task->service.terminalWindow->position.yS;//in the window
				}
			...
			if((i == '\n')  || ((pX + task->service.terminalWindow->font->letterOffset + x) >= task->service.terminalWindow->position.xE) ){	//new string symbol
				...
				if( (i == '\n') || (i == 0xd) )
					task->transmittedData ++;
					//task->transmittedData += (i == '\n')? 1: 0;
				}
//				else	{	//limit of screen coordinates
//					task->transmittedData = task->countOfData; //switch off the task
//					return;
//				}
			//}
			//task->service.terminalWindow->posX
